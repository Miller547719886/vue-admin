...
/* 区域中心--服务商select信息 */
function getOrganizationList(obj, ids) {
    let url = '/user/organization/findAllServiceByOrganizationId'
    if (ids) {
        url += ids.length ? ('?organizationIds=' + ids) : ''
    }
    obj.$http.get(url).then(({ data }) => {
        if (data.code == $HTTP.SUCCESS) {
            if (data.data.length !== 0) {
                data.data.forEach((item, index) => {
                    obj.organizationDataList.push({
                        value: item.id,
                        label: item.name
                    })
                })
